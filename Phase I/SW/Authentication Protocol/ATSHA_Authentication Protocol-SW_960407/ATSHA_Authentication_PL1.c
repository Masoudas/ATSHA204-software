...
PL1_flags ATSHA_Authentication_PL1(unsigned int current_system_time,command_response_struct *pcommand_response)
{
  ...
  static           bool                 authentication_path;                              // A flag that denotes whether the "main" or "auxiliary" path has been taken for authentication.
  ...
  switch (current_PL1_state)    
  {
    ...
    case (READ_CZ_007):       // Reading slot zero of CZ.
      ...
      if (XORstring[0] > XORstring[1])  // If the data previously stored in the XOR array satisfies this constraint.
      {
        next_PL1_state = READ_DZ_207;   // Follow the remainder of the protocol on the main path.
        authentication_path = MAIN_AUTHENTICATION_PATH; // Don't forget the path taken for the next call!
      }
      else
      {
        next_PL1_state = READ_DZ_007a;  // Follow the protocol on the auxiliary path.
        authentication_path = AUXILIARY_AUTHENTICATION_PATH;    
      }
    ...
    case (READ_DZ_107):       // Reading slot one of DZ.
      if (authentication_path == MAIN_AUTHENTICATION_PATH)      // If the main authentication path has been taken, the XOR string is read in the previous state, thus saved here.
      {
        Data_Coding_Scheme(&((*pcommand_response).response[0]), 0x00);      // Encoding the received XOR string data in the previous state. No XORing is required at this stage.
        Swap_Array_Content(&XORstring[0],&((*pcommand_response).response[0]));     // Copy the content of DZ007 read in the previous state (i.e, READ_DZ_007) to XORstring[].
      }
    ...
    case (HOST_AUTHENTICATION):
      Data_Coding_Scheme(&((*pcommand_response).response[0]), 0x00);      // Encoding the result of XOR, which is currently in the response array.
      authentication_flag = Authenticity_Verification(&((*pcommand_response).response[2]),&encoded_CZ007data[0],&XORstring[0]);
      if (authentication_flag == SUCCESSFUL_AUTHENTICATION) 
      {
        ...
        Authenticity_Verification(&((*pcommand_response).command[WC_DATA1_OFFSET]),&encoded_CZ007data[0],&XORstring[0]);    // Compute the updated result of XOR based on the updated XOR strings. Put it in the data byte of the command string so that it is sent out in the next state.
      ...
      {
        current_PL1_state = AUTHENTICATION_FAILURE;     // Else go to the failure state.
        PL1_output_flag = PL1_BUSY;
      }
      break;

    case (WRITE_DZ_107):      // Writing the result of XOR with the updated XOR string.
      Data_Coding_Scheme(&((*pcommand_response).command[WC_DATA1_OFFSET]),0x00);    // Encoding the result of XOR before writing it to ATSHA.
      ...
      diversion_write_counter = (0x07 & XORstring[0]);  // Assigning a random number (which is below 7), which denotes the number of times a diversion string is to be written before writing the XOR string.
      break;       
   
    case (WRITE_DZ_207):    // Writing an updated diversion string in slot 2 of DZ.   
      Data_Coding_Scheme(&encoded_CZ007data[0],XORstring[diversion_write_counter]);        // Repeat the coding scheme with another XOR byte to make a nonsense sequence.
      Assign_WriteCommand_Buffer(pcommand_response, WDZ207_COMMAND, &XORstring[0], WDZ207_COUNT, WDZ207_OPCODE, WDZ207_PARAM1, WDZ207_PARAM20, WDZ207_PARAM21);
    ...
    case (WRITE_DZ_007):       // Updating and writing the XOR string in slot zero of DZ
      Data_Coding_Scheme(&XORstring[0],0x00);        // Encode the XOR string before sending it to ATSHA.
      Assign_WriteCommand_Buffer(pcommand_response, WDZ007_COMMAND, &XORstring[0], WDZ007_COUNT, WDZ007_OPCODE, WDZ007_PARAM1, WDZ007_PARAM20, WDZ007_PARAM21);      
      Assign_Response_Params(pcommand_response,WDZ007_SUCCESSFUL_RESPONSE,WDZ007_RESPONSE_LENGTH,SET_ATSHA_SLEEP);
      current_PL1_state = PL2_BUSY_CYCLE;
      next_PL1_state = WRITE_DZ_207;
      PL1_output_flag = PL1_UPDATE_ATSHA;
      diversion_write_counter = (0x07 & XORstring[1]); // Denotes the number of times a diversion string is to be written after writing the XOR string.
      write_dz_007_flag = XOR_STRING_WRITTEN; // The XOR string is written when entering this state.
      break;  
...
void Retrieve_Status(command_response_struct *pcommand_response, unsigned char *pcurrent_PL1_state, unsigned char *pnext_PL1_state)
{
  ...
  *pcurrent_PL1_state = PL2_BUSY_CYCLE; // Make sure to resend or finish sending the command present in the buffer.
  ...
  {
    *pnext_PL1_state = READ_CZ_007;     // If the last command sent to the chip is a read command, go to the top of the algorithm.
  }
...
bool Authenticity_Verification(unsigned char *pvec1, unsigned char *pvec2, unsigned char *pvec3)
{
  // Initialization
