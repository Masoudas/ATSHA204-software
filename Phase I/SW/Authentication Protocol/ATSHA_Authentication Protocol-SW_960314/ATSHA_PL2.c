...
PL2_flags ATSHA_Operational_Cycle(command_response_struct *command_response, unsigned int current_system_time)
{
     ...
     {     
       ...
       case (CHECK_ATSHA_AWAKE):
         ...
         {
           PL2_Assign_Operation(I2C_status.output_flag, command_response, ATSHA_AWAKE_RESPONSE_LENGTH, CHECK_ATSHA_AWAKE, WAKE_ATSHA, WAKE_ATSHA);           
         }
         PL2_output_flag = PL2_BUSY;
         break;
       ...
       case (SEND_COMMAND):
         ...
         PL2_Assign_Operation(I2C_status.output_flag, command_response, (*command_response).command[1], SEND_COMMAND, READ_RESPONSE, SEND_COMMAND); // Note that recursion operation is unncessary for this mode.
         PL2_output_flag = PL2_BUSY;
         break; 
       ...
       case (READ_RESPONSE):
         ...
         PL2_Assign_Operation(I2C_status.output_flag, command_response, (*command_response).response_length, READ_RESPONSE, (*command_response).sleep_idle, SEND_COMMAND);
       ...
       case (RESET_ATSHA_BUFFER):
         ...
         PL2_Assign_Operation(I2C_status.output_flag, command_response, RESET_COMMAND_LENGTH, RESET_ATSHA_BUFFER, SEND_COMMAND, PL2_RECOVERY_STATUS);     // Recursion operation is unnecessary here.
         PL2_output_flag = PL2_BUSY;
         break;
...
void PL2_Assign_Operation(bool I2C_status, command_response_struct *command_response, unsigned char transaction_length, unsigned char operation_current, unsigned char operation_next, unsigned char recursion_operation)
{
  // Initialization
  ...
  else  // The reception is complete.
  {
    ...
    received_CRC_bytes[1] = (*command_response).response[(*command_response).response_length-1];        // Extract CRC byte two from the response vector (note that the last element of the vector is response_length-1).
    ...
    CRC_check = Check_CRC(2, &((*command_response).response[0]), ((*command_response).response_length-2));  // Check CRC validity using the ACK_NACK module of I2C.
    ...
    else if (current_operation == CHECK_ATSHA_AWAKE && (*command_response).response[1] == ATSHA_AWAKE_RESPONSE)   // Else if a successful response to a wake token has been received
    {
      current_operation = operation_next;       // Just jump to the next operation.
    }
    else if ((*command_response).response[1] == (*command_response).response_status_error && CRC_check == CRC_VALID)     // If status/error byte is response to a successful to a desired command
