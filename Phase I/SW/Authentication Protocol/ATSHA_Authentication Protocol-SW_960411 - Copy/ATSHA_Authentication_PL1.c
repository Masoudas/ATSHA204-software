...
PL1_internal_flags_struct        PL1_internal_flags = {PL1_FINISHED,PL2_READY,FAILED_AUTHENTICATION,0x00,NO_PL2_FAILURE_FLAG,MAIN_AUTHENTICATION_PATH};
...
PL1_flags ATSHA_Authentication_PL1(unsigned int current_system_time,command_response_struct *pcommand_response)
{
  ...
  switch (PL1_data.current_PL1_state)    
  {
    ...
    case (PL2_BUSY_CYCLE):  // Level two of protocol is busy with an operation.
      PL1_internal_flags.PL2_output_flag = ATSHA_PL2(pcommand_response, current_system_time);       // Send the desired command to level 2, assess the status of this level. 
                                                                                                    // Note: This function has been implemented here to render the second implementation approach possible (see the report).
      Change_PL1_State(PL1_internal_flags.PL2_output_flag, &(PL1_data.current_PL1_state), PL1_data.next_PL1_state);     // Assign the next state of PL1 based on the output flag of PL2. 
      break;       
    
    default:  // Facing an unknown state 
      Retrieve_Status(pcommand_response,&(PL1_data.current_PL1_state),&(PL1_data.next_PL1_state));
      break;
