...
I2C_status_struct I2C_Protocol(unsigned char *I2C_data_pointer, unsigned char I2C_data_length, unsigned char I2C_initiation_mode, unsigned char I2C_slave_address, bool check_CRC, unsigned char I2C_bitrate, unsigned char I2C_frequency_prescalar, unsigned int current_system_time)
{
     ...
     {     
       ...
       case (I2C_BUSY):
          ...
          {
           if (I2C_state_timer > I2C_MAX_BUSY_TIME)     // If I2C is busy for more than three ticks.
           {
             I2C_state_timer = 0;
             if (I2C_initiation_mode == I2C_MT_MODE || I2C_initiation_mode == I2C_MR_MODE)
             {    
               Assign_I2C_OutputFlag(I2C_PROTOCOL_BUSY, I2C_MASTER_TIMEOUT_1);            // For the master cases, go to the recovery steps                                      
             }
             else
             {
               // For the slave cases, restart I2C from the top.
       ...
       case (I2C_ILLEGAL_START_STOP):
         I2C_state_timer = 0;
         Assign_I2C_OutputFlag(I2C_PROTOCOL_BUSY, I2C_BUSY);   // Receiving illegal start/stop should theoretically only happen in slave modes, hence just reset the protocol.      
       break;
       ...
       case (I2C_MASTER_TIMEOUT_BUSY):
         I2C_PORT_DIRECTION &= ~I2C_PORT_SDA_PIN;        // Set the SDA pin as input by writing a zero in the DDR register.         
         if ((I2C_PORT_READ & I2C_PORT_SDA_PIN) == I2C_PORT_SDA_PIN)    // if the data in SDA pin is one, we are allowed to end then begin I2C communication.
         {
         ...
         else
         {
           if (current_system_time != previous_system_time)
           {
             unsigned char       pI2C_PORT_WRITE = I2C_PORT_WRITE;      // Save the current values of I2C port.
             I2C_PORT_DIRECTION |= I2C_PORT_SCL_PIN;        // Set the SDA pin as output by writing a one in DDR register.
             I2C_PORT_WRITE = ((~(I2C_PORT_WRITE & I2C_PORT_SCL_PIN)) & I2C_PORT_SCL_PIN) | (pI2C_PORT_WRITE & (~I2C_PORT_SCL_PIN));       // Toggle the content of SCL PIN.
           }
       ...
       case (I2C_MASTER_TIMEOUT_1):
         I2C_PORT_DIRECTION |= I2C_PORT_SDA_PIN;        // Set the SDA pin as output by writing a one in DDR register.
         I2C_PORT_WRITE |= I2C_PORT_SDA_PIN;           // Write a one on this pin.
         Assign_I2C_OutputFlag(I2C_PROTOCOL_BUSY, I2C_MASTER_TIMEOUT_BUSY);    // Check and see if power clocking is required.     
       break;      
