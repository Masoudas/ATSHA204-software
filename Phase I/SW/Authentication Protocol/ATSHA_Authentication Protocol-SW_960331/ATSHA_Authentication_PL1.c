...
PL1_flags ATSHA_Authentication_Protocol(unsigned int current_system_time,command_response_struct *pcommand_response)
{
  ...
  switch (current_PL1_state)    
  {
    ...
    case (READ_CZ_007):       // Reading slot zero of CZ.
      ...
      {
        next_PL1_state = READ_DZ_007;   // Follow the remainder of the protocol on the main path.
      }
      else
      {
        next_PL1_state = READ_DZ_107a;  // Follow the protocol on the auxiliary path.
      }
    ...
    case (READ_DZ_107a):       // Reading slot one of DZ.
      ...
      Assign_ReadCommand_Buffer(*pcommand_response, input_command_name, count_byte, opcode_byte, param1_byte, param20_byte, param21_byte);      
      Assign_WriteCommand_Buffer(pcommand_response, input_command_name, *pdata_string, count_byte, opcode_byte, param1_byte, param20_byte, param21_byte)
      Assign_Response_Params(*pcommand_response, length, successful_response,setSHA_sleep_idle);
