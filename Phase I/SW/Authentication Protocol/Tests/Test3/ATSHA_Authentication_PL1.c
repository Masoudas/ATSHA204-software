...
PL1_flags ATSHA_Authentication_PL1(command_response_struct *pcommand_response,unsigned int current_system_time)
{
  ...
  if (flag = 0x0F && PL1_data.current_PL1_state == AUTHENTICATION_INITIATION)
//  if (flag = 0x0F && PL1_data.current_PL1_state == AUTHENTICATION_FAILURE)    
  {
    flag = 0xFF;
    flag2 = 0xFF;    
    PL1_data.current_PL1_state = 0xFF;
    DDRB = 0x01;
 ...
 //     if ((PL1_internal_flags.authentication_flag == SUCCESSFUL_AUTHENTICATION || PL1_internal_flags.update_flag == UPDATE_INCOMPLETE) && flag2 = 0xFF)  
      {
