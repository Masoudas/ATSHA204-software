...
*@ Version:	1.0
*@ Update:	Last:96/05/26	Previous:96/03/14	Created:96/03/14
...
void Assign_Command_Buffer(command_response_struct *pcommand_response, unsigned char input_command_name, unsigned char word_address, unsigned char count_byte, unsigned char opcode_byte, unsigned char param1_byte, unsigned char param20_byte, unsigned char param21_byte, unsigned char *pdata_string)
{
  //// Initialization
  ...
  (*pcommand_response).command[count_byte-1] = generated_CRC_bytes[1];  // Note that the length of the command vector is count_byte+1;
  (*pcommand_response).command[count_byte] = generated_CRC_bytes[0];  
  
  
  // Assign command length
  (*pcommand_response).command_length = count_byte + 1;        // One being the length of word address!         
