...
PL1_flags ATSHA_Authentication_Protocol(unsigned int current_system_time)
{
  ...
  switch (current_PL1_state)    
  {
    ...
    case (HOST_AUTHENTICATION):
      authentication_flag = Authenticate_Chip(&(command_response.response[2]),&encoded_CZ007data[0],&XORstring[0]);
    ...
    case (WRITE_DZ_107):      // Writing the result of XOR with the updated XOR string.
      Assign_WriteCommand_Buffer(&command_response, &(command_response.command[WC_DATA1_OFFSET]), WDZ107_COUNT, WDZ107_OPCODE, WDZ107_PARAM1, WDZ107_PARAM20, WDZ107_PARAM21); 
