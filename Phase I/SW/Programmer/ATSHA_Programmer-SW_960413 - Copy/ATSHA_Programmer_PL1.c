...
PL1_flags ATSHA_Programmer_PL1(command_response_struct *pcommand_response,unsigned int current_time)
{
  //// Initialization
  ...
  switch (PL1_data.current_PL1_state)    
  {
    ...
    case (READ_LOCKCONFIGS):    // Assessing the state of programming progress by reading the lockbits in configuration zone.     
      ...
      Assign_Response_Params(pcommand_response,RCZ255_SUCCESSFUL_RESPONSE,RCZ255_RESPONSE_LENGTH,SET_ATSHA_SLEEP);
    ...
    case (WRITE_CZ_SLOTCONFIG01):    // Configuring slot 0 and 1 of DZ in CZ.
      ...
      Assign_Response_Params(pcommand_response,WCZ055_SUCCESSFUL_RESPONSE,WCZ055_RESPONSE_LENGTH,SET_ATSHA_SLEEP);
    ...
    case (WRITE_CZ_SLOTCONFIG23):     // Configuring slot 2 and 3 of DZ in CZ.
      ...
      Assign_Response_Params(pcommand_response,WCZ066_SUCCESSFUL_RESPONSE,WCZ066_RESPONSE_LENGTH,SET_ATSHA_SLEEP);
      Assign_PL1_States(PL2_BUSY_CYCLE, LOCK_CZ, PL1_BUSY);            
      break;      
    
    case (LOCK_CZ):        // Locking CZ.
      Assign_LOCKCZ_Buffer(pcommand_response);
      Assign_Response_Params(pcommand_response,LOCKCZ_SUCCESSFUL_RESPONSE,LOCKCZ_RESPONSE_LENGTH,SET_ATSHA_SLEEP);
    ...
    case (LOCK_DZ):     // Locking DZ
      ...
      Assign_Response_Params(pcommand_response,LOCKDZ_SUCCESSFUL_RESPONSE,LOCKDZ_RESPONSE_LENGTH,SET_ATSHA_SLEEP);
    ...
    case (PL2_BUSY_CYCLE):  // Level two of protocol is busy with an operation.
      PL2_output_flag = ATSHA_PL2(pcommand_response, current_time);          // Send the desired command to level 2, assess the status of this level.      
...
void Retrieve_Status(command_response_struct *pcommand_response)
{
  ...
  if ((*pcommand_response).response[2] == 0x55)  // If CZ has not been locked.
  {
