    ...
    //     {         
     ...
     //    }
       ...
       case (CHECK_ATSHA_AWAKE): 
         I2C_status = I2C_Protocol(&((*pcommand_response).response[transception_counter]), 1, I2C_MR_MODE, ATSHA_SLAVE_ADDRESS, IGNORE_CRC, WAKE_TOKEN_I2C_BITRATE, WAKE_TOKEN_I2C_FREQUENCY_PRESCALAR, current_time);
//         if (I2C_status.output_flag == I2C_PROTOCOL_BUSY && tick_counter < 1) // If for the entire duration of a tick, no response to a wake token is received.
//         {
//           current_operation = CHECK_ATSHA_AWAKE;                      
//         }
...
//         {          
 //          tick_counter = 0;
           PL2_Assign_Operation(I2C_status.output_flag, pcommand_response, ATSHA_AWAKE_RESPONSE_LENGTH, CHECK_ATSHA_AWAKE, SEND_COMMAND, CHECK_ATSHA_AWAKE);           
 //        }
         PL2_output_flag = PL2_BUSY;
         break;
