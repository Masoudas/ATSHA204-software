...
unsigned int            previous_time = 0;                      // Previous system time.
unsigned char           a = 0x00;
unsigned char           tick_counter = 0;
PL2_flags ATSHA_PL2(command_response_struct *pcommand_response, unsigned int current_time)
{
     //// Initialization
     ...
     {     
       case (PL2_INITIALIZATION):
         ATSHA_timer = 0;         
         if (previous_command == (*pcommand_response).command_name || (*pcommand_response).command_name == NO_COMMAND)  // If the command to be transmitted has not changed, do not send it.
         {
     ...
     case (WAKE_ATSHA):
       ...
       case (CHECK_ATSHA_AWAKE): 
         tick_counter = 0;
         I2C_status = I2C_Protocol(&((*pcommand_response).response[transception_counter]), 1, I2C_MR_MODE, a, IGNORE_CRC, WAKE_TOKEN_I2C_BITRATE, WAKE_TOKEN_I2C_FREQUENCY_PRESCALAR, current_time);      
         if (I2C_status.current_state == I2C_MR_ADDRESS_NACK)
         {
           a = ATSHA_SLAVE_ADDRESS;                      
         }
        
         PL2_Assign_Operation(I2C_status.output_flag, pcommand_response, ATSHA_AWAKE_RESPONSE_LENGTH, CHECK_ATSHA_AWAKE, SEND_COMMAND, WAKE_ATSHA);           
         PL2_output_flag = PL2_BUSY;
         break;
 
       case (SEND_COMMAND):
         I2C_status = I2C_Protocol(&((*pcommand_response).command[transception_counter]), 1, I2C_MT_MODE, ATSHA_SLAVE_ADDRESS, IGNORE_CRC, 0x00, 0x00, current_time);
       ...
       case (READ_RESPONSE):
         I2C_status = I2C_Protocol(&((*pcommand_response).response[0]), 1, I2C_MR_MODE, ATSHA_SLAVE_ADDRESS, IGNORE_CRC, 0x00, 0x00, current_time);
         ...
         if (transception_counter == 1) // When the reception of first byte is complete, save it as the response length. This must be done, since the length of the response may be different from what was expected.
         {
       ...
       case (SET_ATSHA_SLEEP):
         ...
         I2C_status = I2C_Protocol(&((*pcommand_response).command[0]), 1, I2C_MT_MODE, ATSHA_SLAVE_ADDRESS, IGNORE_CRC, 0x00, 0x00, current_time);
       ...
       case (PL2_RECOVERY_STATUS):       // Recovery state (that follows an unknown state).
         ...
         {
           I2C_status = I2C_Protocol(&(*pcommand_response).response[0], 1, I2C_MR_MODE, ATSHA_SLAVE_ADDRESS, IGNORE_CRC, 0x00, 0x00, current_time); // Note that we only need to finish the current transaction.
       ...
       case (RESET_ATSHA_BUFFER):
         I2C_status = I2C_Protocol(&reset_command, 1, I2C_MT_MODE, ATSHA_SLAVE_ADDRESS, IGNORE_CRC, 0x00, 0x00, current_time);
