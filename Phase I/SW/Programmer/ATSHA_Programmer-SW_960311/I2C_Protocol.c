...
static           unsigned char              I2C_data_counter = I2C_READY;                       // Index of the current I2C data being transmitted.
static           unsigned int               previous_system_time = 0;                           // Previous system time.
static           unsigned char              I2C_state_timer = 0;                                // I2C state timer.
static           I2C_status_struct          I2C_status = {I2C_READY, I2C_PROTOCOL_BUSY};        // Denotes the current status of I2C protocol.
...
I2C_status_struct I2C_Protocol(unsigned char *I2C_data_pointer, unsigned char I2C_data_length, unsigned char I2C_initiation_mode, unsigned char I2C_slave_address, bool check_CRC, unsigned char I2C_bitrate, unsigned char I2C_frequency_prescalar, unsigned int current_system_time)
{
     ...
     {     
       ...
       case (I2C_READY):        // Initiation case.
          ...
          {
            I2C_status.current_state = I2C_BUSY;
            I2C_data_counter = 0;
          ...
          {
            I2C_status.current_state = I2C_READY;                
            I2C_status.output_flag = I2C_PROTOCOL_FINISHED;
          }
       break;       
       ...
       case (I2C_CHECK_CRC):
         ...
         {
           I2C_status.current_state = I2C_READY; 
           I2C_status.output_flag = I2C_PROTOCOL_FINISHED;   // Send out a finished flag.
       ...
       case (I2C_BUSY):
          if ((I2C_CONTROL_REGISTER & (1 << TWINT)) == (0 << TWINT))     // If TWI bit has not changed.
          {
           if (I2C_state_timer > I2C_MAX_BUSY_TIME)     // If the time it has not changed surpasses maximum allowed busy time.
           {
             ...
             if (I2C_initiation_mode == I2C_MT_MODE || I2C_initiation_mode == I2C_MR_MODE)
             {    
               I2C_status.current_state = I2C_MASTER_TIMEOUT_1;
             }
             else
             {
         ...
         {
             I2C_state_timer = 0;
             I2C_status.current_state = I2C_STATUS_REGISTER & I2C_STATUS_REGISTER_PRESCALAR_MASK;            
         }
       break;
       ...
       case (I2C_MASTER_TIMEOUT_BUSY):
         ...
         {
           I2C_status.current_state = I2C_MASTER_TIMEOUT_BUSY; 
         }
         else
         {
           I2C_status.current_state = I2C_MASTER_TIMEOUT_2; 
         }
       break;
       ...
       case (I2C_MT_ADDRESS_ACK):
         ...
         I2C_data_counter++;
         I2C_status.current_state = I2C_BUSY;
         I2C_status.output_flag = I2C_Transmit_Data(I2C_data_pointer, I2C_data_length, I2C_initiation_mode, check_CRC); // The first byte of the data vector is sent out here.
       break;
       ...
       case (I2C_MT_DATA_ACK):
         ...
         I2C_data_counter++;
         I2C_status.current_state = I2C_BUSY;
         I2C_status.output_flag = I2C_Transmit_Data(I2C_data_pointer, I2C_data_length, I2C_initiation_mode, check_CRC);
       break;
       ...
       case (I2C_MR_DATA_ACK):   // An ack is sent in the previous state and now a data is received (again)
         I2C_state_timer = 0;   
         I2C_data_counter++;
         I2C_status.output_flag = I2C_Receive_Data(I2C_data_pointer, I2C_data_length, I2C_initiation_mode, check_CRC);
       break;
...
bool I2C_Transmit_Data(unsigned char *I2C_data_pointer, unsigned char I2C_data_length, unsigned char I2C_initiation_mode, bool check_CRC)
{
  //// Initialization
  bool                  I2C_output_flag = I2C_PROTOCOL_BUSY;
  ...
  else  // Data and/or CRC reception is finished.
  {
    if (I2C_initiation_mode == I2C_MT_MODE)     // If we are in MT mode.
    {
      ...
      I2C_status.current_state = I2C_READY;
      I2C_output_flag = I2C_PROTOCOL_FINISHED;  // This flag is only used for MT mode.
    }
    else // Else, we are in ST mode
    {
  ...
  //// Finalization
  return(I2C_output_flag);
}
#endif

#if GENERATE_MR_MODE == 1 || GENERATE_SR_MODE == 1
bool I2C_Receive_Data(unsigned char *I2C_data_pointer, unsigned char I2C_data_length, unsigned char I2C_initiation_mode, bool check_CRC)
{
  //// Initialization
  ...
  {
    I2C_status.current_state = I2C_CHECK_CRC; 
  }
  ...
  {
    I2C_status.current_state = I2C_READY; 
    I2C_output_flag = I2C_PROTOCOL_FINISHED;   // Send out a finished flag.
    if (I2C_initiation_mode == I2C_MR_MODE)
    {
      I2C_CONTROL_REGISTER = I2C_STOP_SIGNAL;
    }
...
void I2C_Unknown_State(char I2C_initiation_mode)
{
  ...
  {
    I2C_status.current_state = I2C_ST_DATA_ACK; 
  }
