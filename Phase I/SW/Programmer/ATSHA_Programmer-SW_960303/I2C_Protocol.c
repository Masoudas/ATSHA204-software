...
static           unsigned char              I2C_Status = 0;                          // Denotes the current status of I2C protocol.
                                                                                     // For every use (including the very first use, it is set to I2C_READY).


char I2C_Protocol(unsigned char *I2C_data_pointer, unsigned char I2C_data_length, unsigned char I2C_initiation_mode, unsigned char I2C_slave_address, bool check_CRC, unsigned char I2C_bitrate, unsigned char I2C_frequency_prescalar, unsigned int current_system_time)
{
     //// Initialization
     ...
     {     
       ...
       case (I2C_BUSY):
          if ((I2C_CONTROL_REGISTER & (1 << TWINT)) == (0 << TWINT))     // If TWI bit has not changed.
          {
           if (I2C_state_timer > I2C_MAX_BUSY_TIME)     // If the time it has not changed surpasses maximum allowed busy time.
           {
             ...
             if (I2C_initiation_mode == I2C_MT_MODE || I2C_initiation_mode == I2C_MR_MODE)
             {    
               I2C_Status = I2C_MASTER_TIMEOUT_1;
             }
             else
             {
       ...
       case (I2C_MASTER_TIMEOUT_BUSY):
         ...
         {
           I2C_Status = I2C_MASTER_TIMEOUT_BUSY; 
         }
         else
         {
           I2C_Status = I2C_MASTER_TIMEOUT_2; 
         }
       break;
