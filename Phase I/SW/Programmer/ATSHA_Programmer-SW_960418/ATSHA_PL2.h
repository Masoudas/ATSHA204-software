...
#define           CHECK_ATSHA_AWAKE                  0x02              // Wake ATSHA from sleep or idle mode and read the response of ATSHA to see if the answer to wake token has been sent out. 
...
#define           PL2_FINALIZATION                   0x08              // The finalization step of level two.  
#define           PL2_RECOVERY_STATUS                0x09              // The state which allows PL2 to recover itself from an unknown state. 
#define           PL2_TIMEOUT                        0x0A              // The state of watch-dog failsafe timeout of ATSHA   
#define           WAKE_ATSHA                         0x0B              // Wake ATSHA from sleep or idle mode and read the response of ATSHA to see if the answer to wake token has been sent out. 
...
/* Note: The registers are those of I2C (see the SANIC report on I2C protocl).
         Note that the registers are set so that far beyond the wake token condition is satisfied for a clock rate of 8MHz or less.
...
// wake ATSHA command
#define           WAKE_TOKEN_COMMAND                 0x00              // Wake ATSHA command to be sent by I2C (which is sent as an address, yet interpretted by ATSHA as a wake token).
...
Input:
    *pcommand_response: A pointer to command response structure.
     current_time: Denotes the current system time.
