...
PL1_flags ATSHA_Programmer_PL1(command_response_struct *pcommand_response,unsigned int current_system_time)
{
  ...
  if (flag == 0 && PL1_data.current_PL1_state == PROGRAMMING_INITIATION)
  {
    flag = 0xFF;
  ...
  switch (PL1_data.current_PL1_state)    
  {
    case (PROGRAMMING_INITIATION):
      ...
      Assign_Null_Buffer(pcommand_response);    // Assing null command to buffer, so that no further command is transmitted in PL1.
      break;
...
void Change_PL1_State(PL2_flags PL2_output_flag)
{
  ...
  {
    PL1_data.current_PL1_state = PL2_BUSY_CYCLE;        // In case the output flag of level two is unknown, allow the protocol to recover its status.
  }
