...
MACsoftware_output_flag sha204h_mac(struct sha204h_mac_in_out *param)
{
	...
	*p_temp++ = SHA204_SN_1;
	...
	calculate_sha256_param.digest  = param->response;   
	        ???sha204h_calculate_sha256(temporary, SHA204_MSG_SIZE_DERIVE_KEY, 0);             // Initialize SHA256 computation.
        ????sha256_output_param = sha204h_calculate_sha256(temporary, SHA204_MSG_SIZE_DERIVE_KEY, 0);       // Add first 64 bytes of data to SHA256 engine.
...
sha256_output sha204h_calculate_sha256(unsigned char *message, unsigned long int message_len)
{
        ...
        static  unsigned char            SHA_state = SHA_INITIALIZATION;
        ...
        unsigned long int                remainder = (message_len & 0x3F);     // Length of message that does not fit a 64 byte chunck.   
        ...
        switch (SHA_state)
        {
          ...
          case (SHA_FINALIZATION):
            ...
            {
              sha256_update(&ctx, message, remainder);
            }
