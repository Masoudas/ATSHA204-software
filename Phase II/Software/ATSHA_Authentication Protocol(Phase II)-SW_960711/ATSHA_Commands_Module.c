...
void Generate_Nonce(PL1_data_struct *PL1_data, unsigned int current_system_time)
{
  ...
  (*PL1_data).nonce[31] = ((*PL1_data).PL1_data.nonce[15])^PL2_BUSY_CYCLE;    
  ...
  XOR_byte = (unsigned char)(((((*PL1_data).current_system_time) & 0xFF00) >> 8) + (((*PL1_data).current_system_time) & 0xFF));        // Generate a random XOR byte.
