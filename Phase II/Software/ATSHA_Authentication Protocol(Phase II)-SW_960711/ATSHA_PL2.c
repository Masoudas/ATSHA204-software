...
unsigned int            PL2_time_stamp = 0;                     // Denotes a time stamp (can hold when a particular incident happens in PL2).
     ...
     {     
       ...
       case (WAKE_ATSHA):
         ...
         else if (I2C_status.current_state == I2C_MT_ADDRESS_NACK)
         {
           ...
           PL2_time_stamp = ATSHA_timer;       // Save the current time.
         }
         else
         {
           current_operation = WAKE_ATSHA;           
       ...
       case (CHECK_ATSHA_AWAKE):
         ...
         if (I2C_status.current_state == I2C_MR_ADDRESS_NACK && (ATSHA_timer - PL2_time_stamp) >= ATSHA_WAKE_DELAY) // If for the entire duration of a tick, no response to a wake token is received.
         {
           current_operation = WAKE_ATSHA;                      
         }
       ...
       case (SEND_COMMAND):        
         ...
         PL2_Assign_Operation(&I2C_status, pcommand_response, SEND_COMMAND, WAIT_STATE, SEND_COMMAND); // Note that recursion operation is unncessary for this mode.
         PL2_output_flag = PL2_BUSY;
         PL2_time_stamp = ATSHA_timer;      // Calculate the time elapsed since ATSHA started working (this parameter is only used in the wait state).
         break; 
         
       case (WAIT_STATE):       // Wait for ATSHA_COMMANDEXEC_AVETIME ticks before reading the response.
         if ((ATSHA_timer - PL2_time_stamp) <= ATSHA_COMMANDEXEC_AVETIME)
         {
           current_operation = WAIT_STATE;
         }
         else
         {
           current_operation = READ_RESPONSE;           
         }
       ...
       case (KEEP_ATSHA_AWAKE):
         current_operation = PL2_FINALIZATION; // If ATSHA is to be kept awake, send no sleep or idle sequence. Simply terminate PL2.                 
         PL2_output_flag = PL2_BUSY;       
         break;         
